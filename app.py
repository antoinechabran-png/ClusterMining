...
@st.cache_resource
def load_nlp():
    # Only loading English; disabling NER and Parser for max speed
    return spacy.load("en_core_web_sm", disable=["ner", "parser", "attribute_ruler"])

def preprocess_english(text, nlp, custom_stops):
    ...
    for token in doc:
        lemma = token.lemma_
        # Filter: Stopwords, Punctuation, and Custom Exclusions
...
if uploaded_file:
    ...
    if st.button("Generate Map"):
        ...
        with st.spinner("Processing text..."):
            # Clean tokens
            df['tokens'] = df[col].apply(lambda x: preprocess_english(x, nlp, all_stops))
